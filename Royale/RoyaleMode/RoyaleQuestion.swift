...
struct RoyaleQuestion: View {
    ...
    var alert: Alert {
        Alert(title: Text("Quit?"), message: Text("Are you sure you want to leave?"), primaryButton: .destructive(Text("Yes")) {
                self.nav.manager.defaultSocket.emit("winorloss", [-1, self.nav.room])
                self.nav.manager.defaultSocket.disconnect()
    ...
    var body: some View {
        ZStack {
            ...
            if self.screenShown.screenShown {
                ...
                if(self.answeredCorrect) {
                    ...
                    GeometryReader{_ in
                        ...
                        StandingsPopUp(screenShown: self.screenShown, page: self.nav, buttonColor: Color.red, title: "You Are Out!!", buttonText: "Go Home")
            ...
            self.nav.manager.defaultSocket.on("update") { (data, _) in
                self.screenShown.peopleLeft = data[0] as! Int
            }
            self.tl.currentTime = 15.99
            self.isOn = true
            if let item = self.questionData[self.gd.questionNumber] as? [String:Any] {
